...
def create_car_description_prompt(car_info: CarInfo, custom_context: str = "") -> str:
    ...
    prompt = f"""Создай техническое объявление для продажи автомобиля. Фокус на ФАКТАХ и ТЕХНИЧЕСКИХ ДАННЫХ.
...
1. Заголовок: 🚗 [Марка] [Модель] [Год]
2. Custom ID: {car_info.custom_id or 'авто-онлайн'}
3. Технические характеристики (двигатель, КПП, привод, пробег)
4. Состояние и документы
5. Условия продажи (только онлайн, доставка, безнал)
6. Хештеги

ТРЕБОВАНИЯ:
- НИКАКОЙ воды и рекламных фраз
- ТОЛЬКО конкретные технические данные
- Минимум текста, максимум информации
- Деловой тон без эмоций
- Четкая структура с эмодзи-разделителями
- Упор на практичность и техспособности
...
- Связь только через Telegram
- Гарантия производителя

ИЗБЕГАТЬ:
- Эмоциональных оценок ("потрясающий", "невероятный")
